...
> Para una vista general, instalación y configuración, ver `README.md`. Para el historial de versiones, ver `CHANGELOG.md`.
...
- `config.py` → Loader sin dependencias que lee las credenciales desde el archivo `.env` y las expone como `DB_HOST`, `DB_USER` y `DB_PASSWORD`. Utilizado por `conexionMysql.py` y `registroConsultasProductos.py`.
- `.env.example` → Plantilla de credenciales de MySQL. Copiar como `.env` en la misma carpeta y completar. El archivo `.env` está excluido de Git para no exponer las credenciales.
...
- `maquetacion_web.html` → Apunte completo de maquetación de sitios web: qué es maquetar, estructura semántica, modelo de caja, flexbox/grid, layout clásico, responsive, accesibilidad y actividad práctica.
- `actividad_base_alumno.html` → Actividad base tipo landing para completar (Bootstrap 5, carrusel de herramientas y acordeón), con comentarios `COMPLETAR` y `TU TURNO`.
- `img/` → Imágenes usadas por la actividad base (`html.png`, `css.png`, `bootstrap.png`).
